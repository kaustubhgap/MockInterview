"""
Applciation for conducting interviews
"""
...
import torch
import whisper
from helpers import convert_to_string, make_token, speak
from load_whisper import load_stt, transcribe
...
RESPONSE_PROMPT_NAME: str = config["PROMPTS"][
    "response_prompt"
]  # temporary to mimic human
...
RESPONSE_PROMPT: str = ResponseTemplate.Prompts[
...
if __name__ == "__main__":
    ...
    question_agent = QuestionAgent(
        model_name=QUESTION_MODEL_NAME, prompt=INTERVIEW_PROMPT
    )

    # setup answer agent: temporary to mimic a candidate
    answer_agent = ResponseAgent(
        model_name=QUESTION_MODEL_NAME, prompt=RESPONSE_PROMPT
    )
    ...
    all_topics: str = convert_to_string(init_vars, connector="|")
    all_topics: str = "|" + all_topics
    ...
    for init_var in init_vars:
        ...
        topic_conversation["start_time"] = datetime.now().strftime(
            "%m/%d/%Y, %H:%M:%S"
        )
        ...
        while True:
            ...
            generated_question = question_agent.fetch_output(
            ...
            current_question_dict["start_time"] = datetime.now().strftime(
                "%m/%d/%Y, %H:%M:%S"
            )
            current_question_dict["question"][
                "start_time"
            ...
            previous_answer: str = current_answer
